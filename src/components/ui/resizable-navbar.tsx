...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ref={ref}
      className={cn("inset-x-0 top-0 fixed z-50 w-full", className)}
    >
...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "bg-white/70 dark:bg-neutral-950/80 relative z-[60] mx-auto hidden w-full max-w-[85rem] flex-row items-center justify-between rounded-full  px-4 py-2 lg:flex",
        visible && "  border-b border-neutral-200/20 dark:border-white/10",
        className
      )}
    >
      {React.Children.map(children, (child) =>
        React.isValidElement(child) && child.type === NavItems
          ? React.cloneElement(child as React.ReactElement<NavItemsProps>, {
              visible,
            })
          : child
      )}
...
export const NavItems = ({
  ...
  return (
    <div
      className={cn(
        "hidden flex-1 transform flex-row items-center justify-center space-x-2 lg:flex",
        className
      )}
    >
      {items.map((item, idx) => (
        ...
        >
          ...
          >
            ...
            <span
              className={cn(
                "relative z-10 font-semibold text-[15px] transition-colors duration-150",
                hovered === idx || (visible && item.isModal)
                    ...
                    <div key={colIdx} className="flex flex-col flex-shrink-0">
                      ...
                      <div className="flex flex-col gap-y-2">
                        {column.links.slice(0, 5).map((link, linkIdx) => (
                          ...
                          >
                            ...
                            <p className="font-sm  text-neutral-900 dark:text-white">
                              {link.label}
                            </p>
                      ...
                      {column.links.length > 5 && column.viewAllLink && (
                        <button
                          onClick={() => {
                            onItemClick?.();
                            router.push(column.viewAllLink.href);
                          }}
                        ...
                        >
                          <span className="bg-[#9ffb1e]/50 p-2 rounded-md py-1">
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => (
  <motion.div
    ...
    transition={{ type: "spring", stiffness: 200, damping: 50 }}
    className={cn(
      "relative z-50 mx-auto flex w-[calc(100%-1rem)] flex-col items-center justify-between bg-transparent px-4 py-2 lg:hidden",
      visible &&
        "bg-white/80 dark:bg-neutral-950/80 border dark:border-white/10 shadow-lg rounded-full",
      className
    )}
  >
    {children}
  ...
  <AnimatePresence>
    {isOpen && (
      <motion.div
        ...
        className={cn(
          "absolute inset-x-0 top-16 z-50 flex w-full flex-col divide-y divide-gray-200 dark:divide-gray-800 rounded-lg bg-white dark:bg-neutral-950 px-4 py-4 shadow-xl border border-neutral-200 dark:border-neutral-800",
          className
        )}
      >
        {children}
...
export const NavbarLogo = () => {
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
  setHideText((prev) => {
    if (latest > 80 && !prev) return true;
    if (latest <= 60 && prev) return false;
    return prev;
  });
});
  ...
  return (
    <Link
      href="/"
      className="relative z-20 flex items-center gap-2 px-2 py-1 overflow-hidden"
    >
      {/* Logo (always visible) */}
      ...
      {/* Brand name (hides on scroll) */}
      <div
        className="flex items-center overflow-hidden"
        style={{
          width: "120px",
          transform: hideText ? "translateX(-16px)" : "translateX(0)",
          opacity: hideText ? 0 : 1,
          transitionProperty: "transform, opacity",
          transitionDuration: hideText ? "400ms, 200ms" : "400ms, 200ms",
          transitionTimingFunction: "cubic-bezier(0.4, 0, 0.2, 1)",
          transitionDelay: hideText ? "0ms, 180ms" : "0ms, 0ms",
        }}
      >
        <Image
          src={mendygoDark}
